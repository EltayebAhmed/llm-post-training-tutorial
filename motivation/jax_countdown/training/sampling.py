...
from preprocessing import TOKENS, REVERSE_TOKENS, decode_tensor
...
def sample(
    config, model_class, params, input_, padding_token, eos_token, rng, n_tokens, temperature=1.0
):
    ...
    rolling_tokens = jax.random.categorical(rng, logits[:, -1] / temperature, axis=-1)[:, None]
    ...
    for i in range(n_tokens):
        ...
        is_finished = jnp.logical_or(
            is_finished, sampled_tokens == eos_token
        )
...
# @functools.partial(jax.pmap, static_broadcasted_argnums=(0, 2, 4, 5))
# def sample_classic(
#     model, input_, padding_token, rng, n_tokens, temperature=1.0
# ):
#     """Sample from the model using the given input as prompt.
...
#     batch_size = input_.shape[0]
#     seq_len = input_.shape[1]
#     max_seq_len = seq_len + n_tokens + 1
#     assert max_seq_len < 30, (
#         "Initialisation kv_cache crashes"
#         "on seq_len > 30, unknown why. Maybe config issue or a bug in library."
#     )
...
if __name__ == "__main__":
    ...
    stime = time.time()
    results = sample(config, transformers.FlaxGPT2LMHeadModel,
    state['params'], input_, TOKENS["PAD"], rng, 25, 0.01)
    print(results.ravel()[0])
